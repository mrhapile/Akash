...
`ThirdScreen` remains paused and maps pinned scroll progress to queued video seeks. The loader and React coordinate through the internal `portfolio:first-frame-ready` and `portfolio:loader-complete` events.
...
- Verify changes against reduced motion, responsive cropping, first-frame readiness, and scroll-scrub synchronization.
